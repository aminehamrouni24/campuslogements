  ...
  const handleSaveListings = (id) => {
    if (currentUser && currentUser.email) {
      const isSaved = saveListings.some(
        (saveListing) => saveListing._id === id
      );
      if (isSaved) {
        const restListings = saveListings.filter(
          (savedListing) => savedListing._id !== id
        );
      ...
      <div className="card-container">
        ...
        >
          ...
          <div className="absolute bottom-2 left-2 bg-brand-blue py-1 px-2 ">
            <p className="text-xs text-white  font-heading uppercase rounded-sm shadow-sm">
              {type}
            </p>
          </div>
          {offer && (
        ...
        <div className="card_body group-hover:bg-brand-blue/5 duration-500  border-x border-b border-brand-blue/20 ">
          <div
            ...
            onClick={() => navigate(`/listing/${_id}`)}
          >
            <h2 className="text-lg font-heading truncate uppercase duration-300 group-hover:text-brand-blue ">
              {title}
            </h2>
            ...
            <div className="info_container mt-4 flex items-center truncate">
              <p className="font-content text-xs font-bold truncate flex items-center justify-start">
                <FaBed className="mr-1 text-brand-blue" />
                {bed} Bed
              </p>
              <p className="font-content text-xs font-bold truncate flex items-center justify-start ml-4">
          ...
          <div className="listing_footer grid grid-cols-2 align-middle border-t border-brand-blue/40 mt-5 p-3 pb-4">
            <div className="price_container truncate">
              {offer ? (
                <p className="text-xl font-content text-brand-blue font-bold  flex items-center justify-start truncate">
                  ${discountPrice}{" "}
                  <s className="text-gray-400  text-xs mt-1 ml-1">${price}</s>{" "}
                </p>
              ) : (
                <p className="text-xl font-content text-brand-blue font-bold  flex items-center justify-start truncate">
                  ${price}
            ...
            <div className="footer_btn flex items-center justify-end mr-1">
              <button
                ...
                className={`text-lg drop-shadow-sm duration-300  ${
                  heart ? "text-brand-blue" : "text-gray-300"
                } `}
              >
