...
const Header = () => {
  ...
  return (
    <>
      <div className="navbar pl-0 pr-0 pt-3 pb-3   shadow-md" 
      style={{
        backgroundColor:"#fff"      }}
      >
        <div className="px-5  max-w-screen-2xl w-full !mx-auto grid grid-cols-12 gap-1">
          ...
          <div className="col-span-3 sm:col-span-4">
            <h1 className="font-blach sm:text-xl text-sm text-left hover:bg-transparent uppercase text-brand-blue tracking-tighter w-full font-heading font-bold flex items-center justify-start">
              <Link to={"/home"} className="flex items-center justify-start">
                <img className="w-12 h-8" src={myLogo} alt="logo" />
                <span className="hidden sm:block">Campus Logements</span>
              </Link>
            </h1>
          </div>
          ...
          <div className="col-span-3 sm:col-span-5  md:col-span-4 flex items-center justify-end">
            <ul className="hidden sm:ml-5 sm:flex items-center justify-end  pr-4 font-semibold text-brand-blue font-content ">
              ...
              {currentUser ? (
                ...
                <li className="mr-6">
                  <Link
                    to="/login"
                    className=" text-white px-5 font-bold font-headi py-2 rounded uppercase bg-brand-blue text-sm hover:bg-white hover:text-brand-blue duration-300 hover:shadow-sm"
                  >
                    Login
                  </Link>
                </li>
            ...
            <div className="nav_mobile flex items-center justify-center sm:hidden gap-1">
              ...
              <Link
                to={`${currentUser ? "/message" : "/login"}`}
                className="mr-1 text-lg  text-brand-blue"
              >
                <span className="relative">
                  <BsMessenger className="z-10" />
                  {notificationsDB.length === 0 ? (
                    <p
                      className={`text-xs px-[2px] font-heading font-normal bg-lime-600 text-white absolute  top-[-13px] right-[-9px]  flex items-center justify-center rounded-sm`}
                    >
                      new
                    </p>
                  ) : (
              ...
              >
                {isActiveMoblie ? (
                  <MdOutlineClose className="text-red-600 font-bold" />
                ) : (
